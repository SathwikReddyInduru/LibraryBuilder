...
public class BundleService {
	...
	public CloneAtpResult cloneAtpData(Long atpId, Long networkId, String tpName, Long newBucketZoneId) {
		...
		jdbcTemplate.update("""
				...
				values (?,?,?)
				""", newAtpId, oldAtp.get("SERVICE_PACKAGE_DESC").toString().replaceAll("_CL\\d+$","") + "_" + tpName, networkId);
	...
	private Long cloneBundle(Long oldBundleId, Long networkId, String tpName) {
		...
		jdbcTemplate.update("""
				...
				values (?,?,?,?,?,?,?,?,?,sysdate,?,?)
				""", newBundleId, bundle.get("BUNDLE_NAME").toString().replaceAll("_CL\\d+$","") + "_" + tpName, bundle.get("BUNDLE_CHARGE"),
	...
	private void cloneBucket(String oldBucketId, String newBucketId, String tpName, Long networkId,
		...
		jdbcTemplate.update("""
				...
				select
				   ?,
				   REGEXP_REPLACE(BUCKET_NAME,'_CL[0-9]+$','') || '_' || ?,
				   BALANCE_CATEGORY,
				   USAGE_TYPE,
				   VALIDITY_PERIOD_DAYS,
				   BUCKET_UNIT_VALUE,
