...
public class TariffPackageService {
	...
	public List<TariffPackageDetailsDto> getTariffPackages(Integer networkId) {
    ...
    // 1. Primary Tariff Package Details SQL (Updated with start_date)
    String tariffSql = """
       SELECT a.tariff_package_id,
              a.tariff_package_desc,
              ...
              /* DATA BENEFIT */
              CASE
                  WHEN MAX(CASE WHEN f.balance_category = 'DATA' THEN f.total_bucket END) IS NULL
                  THEN NULL
                  WHEN MAX(CASE WHEN f.balance_category = 'DATA' THEN f.total_bucket END) >= 999999999
                  THEN 'UNLIMITED'
                  ELSE fn_data_unit_converter(MAX(CASE WHEN f.balance_category = 'DATA' THEN f.total_bucket END))
              END AS data_benefit,
              /* SMS BENEFIT */
              CASE
                  WHEN MAX(CASE WHEN f.balance_category = 'SMS' THEN f.total_bucket END) IS NULL
                  THEN NULL
                  WHEN MAX(CASE WHEN f.balance_category = 'SMS' THEN f.total_bucket END) >= 999999999
                  THEN 'UNLIMITED'
                  ELSE TO_CHAR(MAX(CASE WHEN f.balance_category = 'SMS' THEN f.total_bucket END)) || ' SMS'
              END AS sms_benefit,
              /* VOICE BENEFIT */
              CASE
                  WHEN MAX(CASE WHEN f.balance_category = 'VOICE' THEN f.total_bucket END) IS NULL
                  THEN NULL
                  WHEN MAX(CASE WHEN f.balance_category = 'VOICE' THEN f.total_bucket END) >= 999999999
                  THEN 'UNLIMITED'
                  ELSE TO_CHAR(MAX(CASE WHEN f.balance_category = 'VOICE' THEN f.total_bucket END)) || ' Sec'
              END AS voice_benefit
         FROM cs_rat_tariff_package a
         LEFT JOIN cs_rat_tariff_service_pack_map c
           ON a.tariff_package_id = c.tariff_package_id
    ...
    // 2. DATP Specific Benefits SQL
    String datpBenefitsSql = """
       SELECT t.tariff_package_id, 
              sp.service_package_id AS datp_id,
              sp.service_package_desc AS datp_name,
              /* VOICE BENEFIT */
              CASE
     ...
     GROUP BY t.tariff_package_id, 
              sp.service_package_id, 
              sp.service_package_desc
     ORDER BY t.tariff_package_id, 
              sp.service_package_desc
       """;

    // 3. Rate Group SQL
    String rateGroupSql = """
       SELECT DISTINCT f.tariff_package_id, 
              f.tariff_package_desc, 
              a.rate_group_name
         FROM cs_rate_group_data a
         JOIN cs_dre_rating_group_details b ON a.rate_group_id = b.rating_group_id
         JOIN cs_rat_service_data_zone_map c ON b.zone_group_id = c.data_zone_id
         JOIN cs_rat_service_plan_package d ON c.service_plan_id = d.service_plan_id
         JOIN cs_rat_tariff_service_pack_map e ON d.service_package_id = e.service_package_id
         JOIN cs_rat_tariff_package f ON e.tariff_package_id = f.tariff_package_id
        WHERE e.network_id = ?
    ...
    List<TariffPackageDetailsDto> tariffList = jdbcTemplate.query(
          ...
          (rs, rowNum) -> {
             ...
             dto.setStartDate(rs.getString("start_date")); // <-- Map start_date here
             dto.setActivationFee(rs.getDouble("activation_fee"));
             dto.setRentalType(rs.getString("rental_type"));
             dto.setRentalPeriod(rs.getLong("rental_period"));
             dto.setDataBenefit(rs.getString("data_benefit"));
             dto.setSmsBenefit(rs.getString("sms_benefit"));
             dto.setVoiceBenefit(rs.getString("voice_benefit"));
             dto.setPackageType(rs.getString("package_type"));
             dto.setIsCorporateYn(rs.getString("is_corporate_yn"));
             dto.setRateGroupNames(new ArrayList<>());
             dto.setDatpBenefits(new ArrayList<>());
             return dto;
          },
          networkId
    );

    // Execute Rate Group Query
    List<Map<String, Object>> rateGroupList = jdbcTemplate.queryForList(rateGroupSql, networkId);

    // Execute DATP Benefits Query
    List<Map<String, Object>> datpRows = jdbcTemplate.queryForList(datpBenefitsSql, networkId);

    // Group Rate Groups by Tariff Package ID
    Map<Long, List<String>> rateGroupMap = new HashMap<>();
    for (Map<String, Object> row : rateGroupList) {
       Long tariffPackageId = ((Number) row.get("TARIFF_PACKAGE_ID")).longValue();
       String rateGroupName = (String) row.get("RATE_GROUP_NAME");
       rateGroupMap.computeIfAbsent(tariffPackageId, k -> new ArrayList<>()).add(rateGroupName);
    }

    // Group DATP Benefits by Tariff Package ID
    Map<Long, List<DatpBenefitDto>> datpBenefitsMap = new HashMap<>();
    for (Map<String, Object> row : datpRows) {
       Long tariffPackageId = ((Number) row.get("TARIFF_PACKAGE_ID")).longValue();
       DatpBenefitDto datp = new DatpBenefitDto();
       datp.setDatpId(((Number) row.get("DATP_ID")).longValue());
       datp.setDatpName((String) row.get("DATP_NAME"));
       datp.setVoiceBenefit((String) row.get("VOICE_BENEFIT"));
       datp.setSmsBenefit((String) row.get("SMS_BENEFIT"));
       datp.setDataBenefit((String) row.get("DATA_BENEFIT"));

       datpBenefitsMap.computeIfAbsent(tariffPackageId, k -> new ArrayList<>()).add(datp);
    }

    // Merge Rate Groups and DATP Benefits into Main Response List
    for (TariffPackageDetailsDto dto : tariffList) {
       dto.setRateGroupNames(rateGroupMap.getOrDefault(dto.getTariff_package_id(), new ArrayList<>()));
       dto.setDatpBenefits(datpBenefitsMap.getOrDefault(dto.getTariff_package_id(), new ArrayList<>()));
    }
